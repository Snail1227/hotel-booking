...
const DatePicker: React.FC<DatePickerProps> = ({ bookingData, setTotalDays }) => {
  const [dateRange, setDateRange] = useState<DateRange>({ checkInDate: null, checkOutDate: null });
  const [currentMonth, setCurrentMonth] = useState(new Date());

  const changeMonth = (num: number) => {
    setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + num, 1));
  };
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        if ((i === 0 && j < firstDayOfMonth) || day > numberOfDays) {
          ...
          const isSelected = dateRange.checkInDate && dateRange.checkOutDate && dayDate >= dateRange.checkInDate && dayDate <= dateRange.checkOutDate;
          week.push(
            <td key={`day-${day}`} className={`day ${isSelected ? 'selected' : ''}`} onClick={() => selectDate(dayDate)}>
              {day}
            </td>
          );
          day++;
  ...
  const selectDate = (date: Date) => {
    ...
    if (!isDatePast) {
      ...
      if (!dateRange.checkInDate || (dateRange.checkInDate && dateRange.checkOutDate) || date < dateRange.checkInDate) {
      ...
      const total = countDaysBetweenDates(newDateRange.checkInDate, newDateRange.checkOutDate);
      setTotalDays(total)
    } else {
  ...
  const countDaysBetweenDates = (startDate: Date | null, endDate: Date | null): number => {
  ...
  return (
    <div className='calendar'>
      <div className='calendar-per-header'>
        ...
        <button className="right-arrow-button" onClick={() => changeMonth(1)}></button>
      </div>
      <table>
        <thead>
          <tr>
      ...
      <div className='calendar-footer'>
        Check-In: {dateRange.checkInDate?.toLocaleDateString()}<br />
