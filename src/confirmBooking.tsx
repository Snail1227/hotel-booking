...
const BookingConfirmation: React.FC = () => {
  ...
  useEffect(() => {
    if (userToken) {
      const fetchUserName = async () => {
        try {
          // Use the non-null assertion operator (!) to assure TypeScript that userToken is not null
  ...
  const formattedCheckInDate = new Date(bookingDetails.checkInDate).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long', 
    day: 'numeric' 
  });

  const formattedCheckOutDate = new Date(bookingDetails.checkOutDate).toLocaleDateString('en-US', {
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  });
